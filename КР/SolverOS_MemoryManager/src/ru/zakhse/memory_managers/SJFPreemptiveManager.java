 ...
 */
public class SJFPreemptiveManager extends MemoryManager {
    public SJFPreemptiveManager(int memorySize, int quantum, ShedullingPolicy policy, MemoryAllocation allocation,
                                Process[]
                                        processes) {
        ...
        while (!isAllProcessFinished(processes)) {
            ...
            for (Process p : waitingProcesses)
                if (pushProcess(allocatedMemory, p, allocation)) {
                    ...
                    toDelete.add(p);
                    if (runningProcesses.element() != runningProcess && runningProcess != null) {
